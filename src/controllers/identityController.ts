...
private checkForMatchingContacts = async (phoneNumber: string, email: string, contactsData: Contact[]): Promise<Contact[] | null> => {
...
private mergeContacts = async (matchedByPhoneNumber: Contact, matchedByEmail: Contact, contactsData: Contact[]) => {
    const olderContact = (new Date(matchedByPhoneNumber!.created_at) < new Date(matchedByEmail!.created_at)) ? matchedByPhoneNumber : matchedByEmail;
